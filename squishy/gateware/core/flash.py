...
class SPIFlash(Elaboratable):
	def __init__(self, *, flash_resource: str, flash_geometry: FlashGeometry, fifo: AsyncFIFO):
	...
	def elaborate(self, platform) -> Module:
		...
		m.submodules.spi = flash = SPIInterface(resource_name = self._flash_resource)
			...
			with m.State('RST'):
				with m.Switch(resetStep):
					with m.Case(0):
						m.d.comb += [
							flash.xfr.eq(1),
							flash.wdat.eq(SPIFlashCmd.RELEASE_PWRDWN),
						]
						m.d.sync += [
							flash.cs.eq(1),
							resetStep.eq(1),
			...
			with m.State('WRITE_EN'):
				with m.Switch(enableStep):
					with m.Case(0):
						m.d.comb += [
							flash.xfr.eq(1),
							flash.wdat.eq(SPIFlashCmd.WRITE_ENABLE),
						]
						m.d.sync += [
							flash.cs.eq(1),
							enableStep.eq(1),
			...
			with m.State('CMD_ERASE'):
				with m.Switch(eraseCmdStep):
					with m.Case(0):
						m.d.comb += [
							flash.xfr.eq(1),
							flash.wdat.eq(platform.flash['commands']['erase']),
						]
						m.d.sync += [
							flash.cs.eq(1),
							eraseCmdStep.eq(1),
					...
					with m.Case(1):
						with m.If(flash.done):
							m.d.comb += [
								flash.xfr.eq(1),
								flash.wdat.eq(self.eraseAddr[16:24]),
							]
					...
					with m.Case(2):
						with m.If(flash.done):
							m.d.comb += [
								flash.xfr.eq(1),
								flash.wdat.eq(self.eraseAddr[8:16]),
							]
					...
					with m.Case(3):
						with m.If(flash.done):
							m.d.comb += [
								flash.xfr.eq(1),
								flash.wdat.eq(self.eraseAddr[0:8]),
							]
			...
			with m.State('ERASE_WAIT'):
				with m.Switch(eraseWaitStep):
					with m.Case(0):
						m.d.comb += [
							flash.xfr.eq(1),
							flash.wdat.eq(SPIFlashCmd.READ_STATUS),
						]
						m.d.sync += [
							flash.cs.eq(1),
							eraseWaitStep.eq(1),
			...
			with m.State('CMD_WRITE'):
				with m.Switch(writeCmdStep):
					with m.Case(0):
						m.d.comb += [
							flash.xfr.eq(1),
							flash.wdat.eq(SPIFlashCmd.PAGE_PROGRAM),
						]
						m.d.sync += [
							flash.cs.eq(1),
							writeCmdStep.eq(1),
					...
					with m.Case(1):
						with m.If(flash.done):
							m.d.comb += [
								flash.xfr.eq(1),
								flash.wdat.eq(self.writeAddr[16:24]),
							]
					...
					with m.Case(2):
						with m.If(flash.done):
							m.d.comb += [
								flash.xfr.eq(1),
								flash.wdat.eq(self.writeAddr[8:16]),
							]
					...
					with m.Case(3):
						with m.If(flash.done):
							m.d.comb += [
								flash.xfr.eq(1),
								flash.wdat.eq(self.writeAddr[0:8]),
							]
